/**
fourtwentyjs-accounts - A suite for managing 420coin accounts in browser.

Welcome to fourtwentyjs-accounts. Generate, encrypt, manage, export and remove 420coin accounts and store them in your browsers local storage. You may also choose to extendWeb3 so that transactions made from accounts stored in browser, can be signed with the private key provided. 420coinJs-Accounts also supports account encryption using the AES encryption protocol. You may choose to optionally encrypt your 420coin account data with a passphrase to prevent others from using or accessing your account.

Requires:
 - cryptojs v0.3.1  <https://github.com/fahad19/crypto-js>
 - localstorejs *  <https://github.com/SilentCicero/localstore>
 - fourtwentyjs-tx v0.4.0  <https://www.npmjs.com/package/fourtwentyjs-tx>
 - fourtwentyjs-tx v1.2.0  <https://www.npmjs.com/package/fourtwentyjs-util>
 - Underscore.js v1.8.3+  <http://underscorejs.org/>
 - Web3.js v0.4.2+ <https://github.com/420integrated/go-420coin/web3.js>

Commands:
    (Browserify)
    browserify --s Accounts index.js -o dist/fourtwentyjs-accounts.js

    (Run)
    node index.js

    (NPM)
    npm install fourtwentyjs-accounts

    (Meteor)
    meteor install silentcicero:fourtwentyjs-accounts
**/

var _ = require('underscore');
var Tx = require('fourtwentyjs-tx');
...
/**
The Accounts constructor method. This method will construct the in browser 420coin accounts manager.
...
var Accounts = module.exports = function(options){
    ...
    var defaultOptions = {
        varName: 'fourtwentyAccounts'
        , minPassphraseLength: 6
        , requirePassphrase: false
        , selectNew: true
        , defaultSmokePrice: 'useWeb3'
...
/**
Prepair 420coin address for either raw transactions or browser storage.
...
Accounts.prototype.set = function(address, accountObject){
    var accounts = LocalStore.get('fourtwentyAccounts');
...
/**
Remove an account from the 420coin accounts stored in browser
...
/**
Generate a new 420coin account in browser with a passphrase that will encrypt the public and private keys with AES for storage.
...
Accounts.prototype.new = function(passphrase){
    ...
    var public = fourtwozeroUtil.privateToPublic(private);
    var address = formatAddress(fourtwozeroUtil.publicToAddress(public)
                                .toString('hex'));
    var accountObject = {
        address: address
        , encrypted: false
        , locked: false
        , hash: fourtwozeroUtil.sha3(public.toString('hex') + private.toString('hex')).toString('hex')
    };
...
Accounts.prototype.get = function(address, passphrase){
    ...
    if(this.isPassphrase(passphrase) && accountObject.encrypted) {
        try {
            ...
            if(fourtwozeroUtil.sha3(accountObject.public + accountObject.private).toString('hex') == accountObject.hash)
...
/**
Clear all stored 420coin accounts in browser.
...
/**
A log function that will log all actions that occur with fourtwentyjs-accounts.
...
Accounts.prototype.signTransaction = function(tx_params, callback) {
    ...
    var rawTx = {
        nonce: formatHex(fourtwozeroUtil.stripHexPrefix(tx_params.nonce)),
        smokePrice: formatHex(fourtwozeroUtil.stripHexPrefix(tx_params.smokePrice)),
        smokeLimit: formatHex(new BigNumber('3141592').toString(16)),
        value: '00',
        data: ''
    };

    if(tx_params.smokePrice != null)
        rawTx.smokePrice = formatHex(fourtwozeroUtil.stripHexPrefix(tx_params.smokePrice));

    if(tx_params.smoke != null)
        rawTx.smokeLimit = formatHex(fourtwozeroUtil.stripHexPrefix(tx_params.smoke));

    if(tx_params.to != null)
        rawTx.to = formatHex(fourtwozeroUtil.stripHexPrefix(tx_params.to));

    if(tx_params.value != null)
        rawTx.value = formatHex(fourtwozeroUtil.stripHexPrefix(tx_params.value));

    if(tx_params.data != null)
        rawTx.data = formatHex(fourtwozeroUtil.stripHexPrefix(tx_params.data));
    ...
    // If the smoke price is zero or null, get the smoke price async
    if(rawTx.smokePrice == '00')
        web3.fourtwenty.getSmokePrice(function(err, result){
            if(err)
                return signTx(err);
            else
                rawTx.smokePrice = formatHex(fourtwozeroUtil.stripHexPrefix(result));
